...
import { registerQnA } from '@/services/qna'
import { RegisterQnaRequest } from '@/types/api'
import logoIcon from '@svgs/logo/challSv2.svg'
import styles from './QnaWindow.module.scss'
...
const ParticipateWindow = () => {
  ...
  const [request, setRequest] = useState<RegisterQnaRequest | null>(null)

  const handleData = () => {
    if (!formTitle) alert('제목을 입력해주세요.')
    else if (!text) alert('내용을 입력해주세요')
    const requestDto: RegisterQnaRequest = {
      qnaTitle: formTitle,
      qnaContent: text,
  ...
  const registerQna = useMutation(registerQnA, {
    onSuccess: (response) => {
    ...
    onError: (error) => {
      ...
      alert('QnA 등록에 실패하였습니다.')
    },
  })
  const handleRegistNotice = () => {
    if (request) registerQna.mutate(request)
  }
  const handleClose = () => {
    setIsOpen(false)
  }
  return (
    <>
      <div className={styles.header}>
        ...
        <div className={styles.header_title}>QnA 등록</div>
      </div>
